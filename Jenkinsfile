...
pipeline {
    ...
    environment {
        ...
        SONARQUBE_PROJECT_KEY = 'fullstack-todo-app'
            ...
            steps {
                sh '''
                    ...
                    javac -d "$WORKSPACE/build/java/classes" \
                      "$WORKSPACE/src/java/com/example/task/TaskPriorityEngine.java"
                '''
            }
        }
        ...
        stage('Run Tests') {
            ...
            steps {
                script {
                    ...
                    int pythonStatus = sh(
                        script: "PYTHONPATH=\"$WORKSPACE/src/backend\" python3 -m pytest \"$WORKSPACE/tests/python/test_task_service.py\" \"$WORKSPACE/tests/python/test_task_utils.py\" --json-report --json-report-file=\"$WORKSPACE/$AI_REPORTS_DIR/python_test_results.json\" > /dev/null 2>&1",
                        returnStatus: true
                    )
                    if (pythonStatus != 0) {
                        failedSuites << 'python'
                    ...
                    int jsStatus = sh(
                        script: "node --test --test-reporter=junit --test-reporter-destination=\"$WORKSPACE/$AI_REPORTS_DIR/js_test_results.xml\" tests/javascript/test_app.js > /dev/null 2>&1",
                        returnStatus: true
                    )
                    if (jsStatus != 0) {
